...
namespace Input.Jobs
{
    ...
    public partial struct InputUpdateJob : IJobEntity
    {
        ...
        public void Execute(ref UserMouseInfo userMouseInfo, Entity entity)
        {
            ...
            UpdateComponentWithValue<UserMovement, UserFloat2InputValue, float2>(ECB, entity, InputType.Move, new UserFloat2InputValue
            {
                Value = Movement
            });
            UpdateComponentWithValue<UserScroll, UserIntInputValue, int>(ECB, entity, InputType.Scroll, new UserIntInputValue
            {
                Value = MouseScroll
            });
            UpdateComponentWithValue<UserMouseMovement, UserFloat2InputValue, float2>(ECB, entity, InputType.MouseMove, new UserFloat2InputValue
            {
                Value = MouseMovement
            });
        ...
        private void UpdateComponentWithValue<T, T1, T2>(EntityCommandBuffer ecb, Entity entity, InputType inputType, T1 value)
